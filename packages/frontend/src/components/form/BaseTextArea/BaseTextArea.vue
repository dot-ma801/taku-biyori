...
const errors = computed(() => {
  ...
  return props.rules.map(r => r(model.value)).filter(r => r !== true) as string[]
})
</script>
...
<template>
  <div class="textarea-wrap">
    ...
    />
    <span v-if="hint && errors.length === 0" class="textarea-wrap__hint">{{ hint }}</span>
    <div v-if="errors.length > 0" class="textarea-wrap__errors">
      <span v-for="error in errors" :key="error" class="textarea-wrap__error">{{ error }}</span>
    </div>
  </div>
</template>
