...
description: >
  PR に付いたレビュー指摘に対応するときに使う。
  「レビューコメントに対応して」「指摘を直して」「PR #N のレビューを見て」
  「CodeRabbit の指摘を潰して」などの依頼が対象。
  人間・AI エージェント・レビュー bot のいずれの指摘も投稿者を問わず対象に含める。
targets:
  - '*'
---
# レビュー指摘への対応
...
**最終 push と CI 完了のあとに、Step 2 の GraphQL クエリをもう一度実行する。**
対応中や push 後に、人間やレビュー bot が新しい指摘を投稿していることがある。
初回の対応表は固定したまま進めてよいが、それを完了条件にはしない。

新規の unresolved スレッドが出たら Step 3 に戻る。**0 件になるまで繰り返す。**

## チェックリスト

- [ ] 対象 PR のブランチに切り替えてから作業した
- [ ] レビュースレッド（GraphQL）と PR 全体コメントの両方を集めた
...
- [ ] コミットのたびに `pnpm check` を通した
- [ ] 最終 push と CI 完了後に未解決スレッドを再取得し、新規分が 0 件になった
- [ ] CI が通っている
