...
describe('BaseProgress', () => {
  ...
  describe('variant', () => {
    it.each(['default', 'success', 'warning', 'error'] as const)(
      ...
      (variant) => {
        ...
        // Assert
        expect(wrapper.find('.progress-wrap').classes()).toContain(
          `progress--${variant}`,
        );
      },
    );
  ...
  describe('indeterminate', () => {
    it('indeterminate=false のとき .progress--indeterminate クラスが付与されない', () => {
      ...
      // Assert
      expect(wrapper.find('.progress-wrap').classes()).not.toContain(
  ...
  describe('label', () => {
    it('label prop を渡したときラベルテキストが表示される', () => {
      ...
      // Assert
      expect(wrapper.find('.progress-wrap__label').text()).toBe(
        'アップロード中',
      );
    });
  ...
  describe('アクセシビリティ', () => {
    ...
    it('aria-valuemin="0" が付与されている', () => {
      ...
      // Assert
      expect(
        wrapper.find('[role="progressbar"]').attributes('aria-valuemin'),
      ).toBe('0');
    });
