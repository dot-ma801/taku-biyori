...
**カードの主役はステータスで入れ替える**（出し分けは `canViewSharedPlayMemos(status)` ひとつ）。

| ステータス | 主役 | カードの中身 |
|---|---|---|
| `draft` / `confirmed` / `today` | **書く** | 公開状態バッジ・本文2行のプレビュー・文字数と最終保存時刻・「続きを書く」。**カード全体がメモ画面へのリンク**（プレイ中に何度も開くため、押す場所を1つに固定する） |
| `completed` / `cancelled` | **読む** | 公開状態バッジ・**公開しているメンバーのチップ列**（アバター + 名前。押すとその人のメモへ直行）・「メモを開く」 |

| 表示要素 | 表示条件 | 内容 |
|---|---|---|
| プレイメモのカード | ログインユーザー かつ その卓のメンバー | 上表のとおり |
| 公開が1件も無いときの代替 | メンバー かつ `completed` / `cancelled` かつ 公開ゼロ | 「メモを公開すると、ほかのメンバーが読めるようになります」+「メモを開いて公開する」 |
| 読む導線 | メンバー以外 かつ 公開メモが1件以上ある | 「公開されたプレイメモが N 件あります」+ メモ画面へのリンク |
| ログイン導線 | 未ログイン または ゲスト | 「メモを書けるのは、ログインしているメンバーだけです」+ ログインリンク（要求 §4） |
| （何も出さない） | ログイン済みの非メンバー かつ 読む公開メモも無い | セクションごと非表示。`GET .../play-memos/me` も呼ばない |

- **当日はセクションの並び順を上げる。** `today` のときだけ備考（`MemoDisplay`）より上、ステータス表示の
  直下に置く。プレイ中は「すぐ開けること」が最優先で、スクロールの奥にあると入口として機能しない。
  CSS の `order` ではなく描画位置そのものを変える（見た目とフォーカス順を一致させるため）
- 公開メンバーのチップは `?member=<memberId>` 付きで飛ばす。卓詳細から目的の人のメモまで1操作で着く。
  サイドバーと役割が重複するように見えるが、**入口＝誰のを読むか選ぶ / サイドバー＝読みながら切り替える**で分かれる
- 読む導線は**メンバー以外にも**出す。完了・中止した卓の公開メモは未ログイン・ゲストも読めるため、
  「書けません」だけを伝えて終わらせない（読む場所はメモ画面に集約している以上、入口がないと辿り着けない）
- **公開状態がひと目で分かる**表示にする（要求 §4）。バッジは `shared_at` の有無から導く
- 完了・中止で本文が編集できなくなる案内（要求 §4）は**メモ画面（書く面）に置く**。
  実際に書いている場所で伝わればよく、入口のカードに常時出すと読まれない注意書きだけが場所を取る
