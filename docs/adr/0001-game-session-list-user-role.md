...
> **補足（[ADR 0008](./0008-guest-account-linking.md)）**: ホスト承認による紐づけを導入したため、
> 紐づけ済みのメンバーは `user_id` が入り `'member'` として扱われる。`role` に `'guest'` を
> 追加しない方針は変わらない。
...
| ゲスト参加とログインユーザーの紐づけ | 設計コストが高く優先度が低い | ~~ゲスト UX 改善の議論が起きたタイミング~~ → [ADR 0008](./0008-guest-account-linking.md) で決定済み |
