...
describe('requestMemberLink', () => {
  ...
  it('申請は対象メンバーと申請者のユーザーIDで登録される', async () => {
    ...
    // Act
    await requestMemberLink(repo, GAME_SESSION_ID, MEMBER_ID, USER_ID);
  ...
  it('存在しない卓IDは notFound を返す', async () => {
    // Arrange
    const repo = makeRepo({
      findGameSessionVisibility: vi.fn().mockResolvedValue(null),
    });
  ...
  it('申請者が既にその卓のメンバーなら alreadyMember を返す', async () => {
    // Arrange
    const repo = makeRepo({
