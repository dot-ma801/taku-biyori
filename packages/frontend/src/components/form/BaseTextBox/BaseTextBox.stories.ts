...
export const Default: Story = {
  ...
  render: (args) => ({
    ...
    template: '<BaseTextBox v-bind="args" v-model="value" style="max-width: 320px;" />',
  }),
}

export const WithHint: Story = {
  args: { label: 'メールアドレス', placeholder: 'example@email.com', hint: '登録済みのメールアドレスを入力してください', type: 'email' },
  render: (args) => ({
    components: { BaseTextBox },
    setup() {
      const value = ref('')
...
export const WithValidation: Story = {
  ...
  render: (args) => ({
    ...
    setup() {
      const value = ref('')
      const rules = [(v: string) => v.length >= 8 || '8文字以上で入力してください']
      return { args, value, rules }
    },
    template: '<BaseTextBox v-bind="args" v-model="value" :rules="rules" style="max-width: 320px;" />',
  }),
}
