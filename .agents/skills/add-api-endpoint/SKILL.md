...
description: >
  taku-biyori のバックエンドを実装・変更するときに使う。 「API を追加して」「エンドポイントを実装して」「POST /api/...
  を作って」 「このリソースの取得 API がほしい」に加え、ユースケース・ドメイン・リポジトリだけを 足す（ルートを増やさない）変更も対象。 shared
  の型定義 → テスト → 層ごとの実装 → ルート登録の順で TDD で進める。
---
# API エンドポイントの追加

`packages/backend` に Hono のルートを1本増やすまでの手順。**TDD（Red → Green → Refactor）で進める。**

**ルートを増やさない backend の変更**（ユースケース・ドメイン・リポジトリだけを足す／直す）も
この手順に従う。違いは 5（ルート登録）を飛ばすことだけ。
