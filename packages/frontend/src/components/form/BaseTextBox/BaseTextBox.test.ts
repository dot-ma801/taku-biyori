...
describe('BaseTextBox', () => {
  ...
  describe('label', () => {
    it('label prop を渡したときラベルテキストが表示される', () => {
      // Arrange & Act
      const wrapper = mount(BaseTextBox, { props: { label: 'メールアドレス' } })
  ...
  describe('hint', () => {
    it('hint prop を渡したときヒントテキストが表示される', () => {
      // Arrange & Act
      const wrapper = mount(BaseTextBox, { props: { hint: 'ログインに使用します' } })

      // Assert
      expect(wrapper.find('.textbox__hint').text()).toBe('ログインに使用します')
    })
  })
  ...
  describe('modelValue', () => {
    it('modelValue の値が input に反映される', () => {
      // Arrange & Act
      const wrapper = mount(BaseTextBox, { props: { modelValue: 'test@example.com' } })

      // Assert
      expect((wrapper.find('.textbox__control').element as HTMLInputElement).value).toBe(
        'test@example.com',
      )
    })
  ...
  describe('disabled / readonly', () => {
    it('disabled=true のとき input が入力不能になる', () => {
      ...
      // Assert
      expect((wrapper.find('.textbox__control').element as HTMLInputElement).disabled).toBe(true)
    })
    ...
    it('readonly=true のとき input が読み取り専用になる', () => {
      ...
      // Assert
      expect((wrapper.find('.textbox__control').element as HTMLInputElement).readOnly).toBe(true)
    })
  })
})
