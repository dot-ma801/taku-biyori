...
// 自分の回答（「あなたの回答」行の表示用）
function myAnswer(date: AvailabilityDate): Answer | null {
  ...
  <div v-if="hasDates" class="card-list">
    ...
    <div v-for="date in availabilityDates" :key="date.id" class="card">
      <div class="card-header">
        ...
        <span class="tally">
          <span class="tally-item tally-item--ok"
            >◯{{ answerCounts(date, members).ok }}</span
          >
          <span class="tally-item tally-item--maybe"
            >△{{ answerCounts(date, members).maybe }}</span
          >
          <span class="tally-item tally-item--ng"
            >×{{ answerCounts(date, members).ng }}</span
          >
        </span>
      </div>
