...
| プレイメモの公開切替 | 本人 | 常時 |
| 公開メモの閲覧 | **誰でも**（未ログイン・ゲストを含む） | セッションが `completed` または `cancelled` かつ ロビーが公開済み（`publishedAt != null`） |

**この表は文章ではなく `shared` のポリシーテーブルとして実装する。** 置き場所は §4-5 を参照。

> 「公開メモの閲覧」だけは**閲覧者のロールに依存しない**（要求 §3-4）。ロールを引数に取る
> ポリシーテーブルに載せると「着席者だけ」と読めてしまうため、ステータス単独の関数
> `canViewSharedPlayMemos()` として分けている。ロビーの公開判定はサーバ側で先に噛ませる（§6-13-4）。
...
| ステップ | 旧 | v2 |
|---|---|---|
| 1 | 候補日選択 | 開催日の決定（**候補日から選ぶ の1経路のみ**） |
| 2 | 参加者選択 | 着席者の選択（`ok`/`maybe` を既定選択、`ng`/未回答は警告） |
| 3 | 確認 | 確認 + 任意の上書き（呼び名・場所・時間帯・当日連絡） |

- 「確定後は取り消せません」という文言は**削除する**（取り消せるし、何度でも開ける）
- ステップ1の「直接日付を入れる」経路は**持たない**。この導線は
  「日程調整の結果を確定する」操作であり、調整と無関係な日付を入れられると
  意味がねじれる。日程が決まっている卓は §7-1 の作成画面（「開催日を入れる」）で立てる
- 定員ミスマッチは別ダイアログ（`CapacityMismatchDialog`）ではなく、
  確認ステップ内の注意書きで伝える。ステップを止めるほどの事柄ではない
- 確定と同時に**受付を閉じる**（`status` が `open` のときだけ `closed` にする）。
  確定後に増えた参加者は当日の参加者に入っておらず、ホストが気づけないため。
  閉じ損ねても確定は成立させる（ホストは「追加募集」で開き直せる）
- 確定の導線（日程調整タブの「日程を確定する」）は、**確定待ちの調整があるときだけ**出す。
  ロビーのステータスだけで判定すると確定後も押せたままになり、開催が二重にできる
- 成功後も画面は変えない（#152）。この場で「調整中 → 開催予定」に切り替わる
