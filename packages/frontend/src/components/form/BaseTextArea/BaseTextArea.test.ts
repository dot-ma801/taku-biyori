...
describe('BaseTextArea', () => {
  ...
  describe('label / placeholder / hint', () => {
    ...
    it('hint prop を渡したときヒントテキストが表示される', () => {
      // Arrange & Act
      const wrapper = mount(BaseTextArea, {
        props: { hint: '最大 1000 文字' },
      });

      // Assert
      expect(wrapper.find('.textarea-wrap__hint').text()).toBe(
        '最大 1000 文字',
      );
    });
  });
  ...
  describe('resize', () => {
    it.each(['none', 'vertical', 'horizontal', 'both'] as const)(
      ...
      (resize) => {
        ...
        // Assert
        expect(wrapper.find('textarea').attributes('style')).toContain(
          `resize: ${resize}`,
        );
      },
    );
