...
describe('getGuestLink', () => {
  it('ホストがリクエストすると ok とトークンを返す', async () => {
    ...
    // Act
    const result = await getGuestLink(repo, 'lobby-1', 'user-1');
  ...
  it('ホスト以外がリクエストすると forbidden を返す', async () => {
    ...
    // Act
    const result = await getGuestLink(repo, 'lobby-1', 'other-user');
  ...
  it('findGuestLinkInfo に id を渡す', async () => {
    ...
    // Act
    await getGuestLink(repo, 'lobby-xyz', 'user-1');

    // Assert
    expect(findGuestLinkInfo).toHaveBeenCalledWith('lobby-xyz');
  });
});
