...
describe('approveMemberLink', () => {
  it('ホストが承認するとゲスト行にユーザーIDが入る', async () => {
    ...
    // Act
    const result = await approveMemberLink(repo, LOBBY_ID, REQUEST_ID, HOST_USER_ID);
  ...
  it('存在しない申請IDは notFound を返す', async () => {
    ...
    // Act
    const result = await approveMemberLink(repo, LOBBY_ID, 'nonexistent', HOST_USER_ID);
  ...
  it('ホスト以外は forbidden を返す', async () => {
    ...
    // Act
    const result = await approveMemberLink(repo, LOBBY_ID, REQUEST_ID, 'other-user');
