...
**人間が明示的に呼ぶのは `/dev-flow` と `/requirement-dig` の2つだけ**です。残りは依頼の文面から
...
flowchart TD
    ...
    A["1. 要望を詰める ・ requirement-dig<br/>呼ぶ: 人間<br/>止まる: 共通理解を確認するまで実装に移らない"]
...
画面は処理ロジックを `tdd-composable` で先に固めてから `.vue` を書き、**e2e が緑になるまでを1段**とします
（`add-feature-screen`）。
