...
async function submit() {
  ...
  try {
    ...
    errorMessage.value = error instanceof ApiError ? error.message : '開催の作成に失敗しました';
  } finally {
    loading.value = false;
  }
}
...
<template>
  <PageContainer size="sm">
    <form class="form" @submit.prevent="submit">
      ...
      <p class="description">受付を開かないロビーと開催をまとめて作成します。</p>
      <BaseAlert v-if="errorMessage" variant="error">{{ errorMessage }}</BaseAlert>
      <BaseTextBox v-model="title" label="呼び名" required />
      <BaseDatePicker v-model="scheduledAt" label="開催日" required disable-past />
      ...
      <div class="actions">
        <BaseButton type="button" variant="secondary" @click="router.back()">キャンセル</BaseButton>
...
.description { margin: 0; color: var(--color-text-muted); }
