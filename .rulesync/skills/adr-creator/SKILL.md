...
description: |
  ...
  ただし書く前に、ADR にすべき判断かどうかを3条件で判定する。満たさないものは書かずに行き先へ回す。
targets:
  - '*'
---
# ADR Creator
...
### Step 0: 書くべきか判定する（省略不可）

**ADR は決めたことすべてに書くものではない。** 次の3条件を**すべて**満たすときだけ書く。

| 条件 | 判定の問い |
|---|---|
| 元に戻しにくい | 覆すのにコードだけでなく、データや外部との契約まで作り直しが要るか |
| 文脈がないと驚かれる | 半年後の読み手が「なぜこうなっている？」と手を止めるか |
| 本物のトレードオフの産物 | 棄却した案にも相応の利点があったか |

1つでも欠けたら ADR にせず、行き先へ回す。
...
その旨と行き先の案を伝える。** そのうえで書くよう求められたら書く。
...
### Step 4: ファイル書き出し → レビュー依頼

ADR のドラフトが完成したら、**まず `docs/adr/` にファイルとして書き出す**。
チャットにドラフト全文を貼り付けない。

書き出し後、ファイルパスをリンク形式でユーザーに伝え、ファイルを開いて確認するよう促す。

```
[docs/adr/0003-frontend-routing-structure.md](docs/adr/0003-frontend-routing-structure.md) を作成しました。確認をお願いします。
```
