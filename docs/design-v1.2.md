...
**参加メンバー全員**を並べ、読めない相手はタグで理由を示す。**どの行も選べる**（読めない行は
本文の代わりに読めない理由を出す）。
...
| 他メンバー（非公開） | 非公開 | 可 | 「このメモは公開されていません」 |
| ゲストメンバー | ゲスト | 可 | 「ゲストのメンバーはプレイメモを持てません」＋書けるのはログインメンバーだけである旨 |

- タグの語彙は **公開 / 非公開 / ゲスト** の3つ。「作成不可」のような禁止の言い方はしない
  （ゲストがメモを持てないのは制裁ではなく本人確認の手段がないという仕様上の帰結）
- 読めない行を `disabled` にしない。押しても何も起きない項目は理由を伝える場所が無く、
  「壊れている」と読まれる。押せるようにして、本文の位置に読めない理由を出す
- サイドバーの各行と本文側の見出しに、誰のメモかが分かるようメンバーのアバター（`UserAvatar`）を出す。
  絵柄の種は他画面と同じく `user_id` を優先し、持たないゲストは素の名前にフォールバックする
...
- 選択中のメンバーは `?member=<memberId>` で URL に載せる。指定が無ければ、メンバーは自分、
  それ以外は先頭の公開メモ（**既定では読めない相手を開かない**）。
  卓に居ないメンバー ID を指定された場合だけ既定の選択へ `replace` で落とす
- 完了・中止していれば、公開しているメンバーが1人も居なくてもサイドバーを出す
  （「誰も公開していない」ことも情報であり、行を押せば理由も読めるため）
- モバイルではサイドバーを畳み、横スクロールのチップ列として本文の上に置く。
  1件が幅を埋めきらないようチップに上限（60%）を置き、2件目が見切れる形で続きがあることを見せる
- 見出し（プレイメモ／メンバー名）と卓名は、狭い画面では縦に積んでそれぞれ1行で省略する

#### 保存

- 保存は**明示的な操作のみ**（自動保存はしない）。ユーザーが保存ボタンを押したときだけ `PUT` する
- **未保存のまま離脱しようとしたら警告する。** アプリ内の遷移は `onBeforeRouteLeave`、
  リロード・タブを閉じる・外部サイトへの移動は `beforeunload` で拾う
- サイドバーの切り替えはクエリ変更なので `onBeforeRouteLeave` が発火しない。
  `onBeforeRouteUpdate` でも同じ警告を出す
...
### 本文の保存は明示的な操作のみとし、自動保存はしない（2026-08-03 追記）

一度は「3〜4時間書き続ける前提では、保存ボタン1つだとリロードやタブ落ちで全部失われる」
という理由でデバウンス自動保存を採ったが、**作者判断で撤回した**。

メモは自分用の記録であり、いつサーバに送るかはユーザーが決める方が素直である。
自動保存は「送信中に書き足した分の扱い」「タイマーと再取得の競合」「離脱時に flush するか
確認するか」といった状態を増やし、実際に実装した際も送信中の入力が消える不具合と
二重送信の不具合を生んだ（レビューで検出・修正済み）。得られる安全性に対して状態機械が重すぎる。

代わりに、**未保存のまま離脱しようとしたときの警告**でデータ欠損を防ぐ。
アプリ内の遷移は `onBeforeRouteLeave`、リロード・タブを閉じる・外部サイトへの移動は
`beforeunload` で拾う。どちらも `isDirty` だけを見れば判定できるため、追加の状態を持たない。

`409`（書いている最中に卓が完了・中止した）は仕様どおりのエラーなのでリトライせず
読み取りへ落とす。先回りしてステータスを監視すると状態が二重になるため、返ってきてから落とす。
