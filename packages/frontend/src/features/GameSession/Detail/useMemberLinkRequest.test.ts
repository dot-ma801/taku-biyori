...
vi.mock('@/api/game-session', () => ({
  requestGameSessionMemberLink: vi.fn(),
}));
...
const guestMember: GameSessionMember = {
  id: 'member-guest-1',
  userId: null,
  userName: null,
  guestName: 'ゲスト太郎',
...
const loggedInMember: GameSessionMember = {
  id: 'member-user-1',
  userId: USER_ID,
  userName: 'たろう',
  guestName: null,
...
describe('submit', () => {
  it('選択したゲスト行で申請し、成功を通知して所有者へ通知する', async () => {
    // Arrange
    vi.mocked(requestGameSessionMemberLink).mockResolvedValue({
      id: 'request-1',
    ...
    // Assert
    expect(requestGameSessionMemberLink).toHaveBeenCalledWith(
      GAME_SESSION_ID,
      guestMember.id,
    );
  ...
  it('未選択のときは API を呼ばずにエラーを通知する', async () => {
    ...
    // Assert
    expect(requestGameSessionMemberLink).not.toHaveBeenCalled();
    expect(mockToastError).toHaveBeenCalled();
  });

  it('API が失敗したらエラーを通知し、所有者へは通知しない', async () => {
    // Arrange
    vi.mocked(requestGameSessionMemberLink).mockRejectedValue(new Error('failed'));
    const onRequested = vi.fn();
    const { selectedMemberId, submit } = useMemberLinkRequest(
      GAME_SESSION_ID,
      () => [guestMember],
  ...
  it('送信中の二重実行は無視する', async () => {
    ...
    vi.mocked(requestGameSessionMemberLink).mockReturnValue(
      new Promise((r) => {
        resolve = r as (v: unknown) => void;
      }) as ReturnType<typeof requestGameSessionMemberLink>,
    );
    const { selectedMemberId, submit } = useMemberLinkRequest(
      GAME_SESSION_ID,
      () => [guestMember],
    ...
    // Assert
    expect(requestGameSessionMemberLink).toHaveBeenCalledTimes(1);
  });
});
