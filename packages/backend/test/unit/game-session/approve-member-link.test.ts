...
import type { ApproveMemberLinkRepository } from '@/game-session/application/approve-member-link';
...
describe('approveMemberLink', () => {
  ...
  it('ホスト以外の場合は紐づけを実行しない', async () => {
    ...
    // Act
    await approveMemberLink(repo, GAME_SESSION_ID, REQUEST_ID, 'other-user');
