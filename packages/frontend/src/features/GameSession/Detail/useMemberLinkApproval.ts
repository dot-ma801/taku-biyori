...
export const useMemberLinkApproval = (
  ...
  onApproved: (member: GameSessionMember) => void,
) => {
  const toast = useToast();
  const loading = ref(false);

  /** 承認待ちの申請。この composable が所有する */
  const requests = ref<GameSessionMemberLinkRequest[]>([]);
  ...
  async function load() {
    ...
    try {
      requests.value = await listGameSessionMemberLinkRequests(gameSessionId);
    } catch {
  ...
  async function approve(requestId: string) {
    ...
    loading.value = true;
    try {
      const member = await approveGameSessionMemberLink(gameSessionId, requestId);
  ...
  async function reject(requestId: string) {
    ...
    loading.value = true;
    try {
      await deleteGameSessionMemberLinkRequest(gameSessionId, requestId);
      removeRequest(requestId);
