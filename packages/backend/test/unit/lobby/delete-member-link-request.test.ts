...
describe('deleteMemberLinkRequest', () => {
  it('ホストは申請を却下できる', async () => {
    ...
    // Act
    const result = await deleteMemberLinkRequest(repo, LOBBY_ID, REQUEST_ID, HOST_USER_ID);
  ...
  it('ホストでも申請者でもないユーザーは forbidden を返す', async () => {
    ...
    // Act
    const result = await deleteMemberLinkRequest(repo, LOBBY_ID, REQUEST_ID, 'other-user');
  ...
  it('存在しない申請IDは notFound を返す', async () => {
    ...
    // Act
    const result = await deleteMemberLinkRequest(repo, LOBBY_ID, 'nonexistent', HOST_USER_ID);
