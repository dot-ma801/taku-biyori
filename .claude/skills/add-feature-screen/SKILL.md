...
description: >
  ...
  「画面を作って」「タブを追加して」「詳細画面に〇〇を出して」「この一覧を直して」 などの依頼が対象。状態の列挙 → ロジックは composable へ →
  組み立て → e2e で止める、の順で進める。 `components/` の基本 UI は add-basic-component、composable
  単体は tdd-composable の担当。
---
# features の画面を組む
