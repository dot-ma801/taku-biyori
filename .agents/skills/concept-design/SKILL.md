...
`docs/concept/CONTEXT.md` は**対話中に育てるファイル**であり、最後にまとめて書くものではない。

- **食い違いはその場で指摘する。** 用語集に無い言葉や、用語集と違う意味で既存の語が使われたら、
  対話を先へ進める前に「その言葉は用語集ではこう定義されている」と突き合わせる。曖昧なまま進まない
- **決着したらその場で追記する。** 新しい概念名・ロール・紛らわしい語の対比が固まった時点で
  CONTEXT.md を更新する。Step 6 まで溜めない
- **コードとも突き合わせる。** ユーザーが語る振る舞いが実装と食い違っていたら、どちらが正かを聞く
...
- 用語集（`docs/concept/CONTEXT.md` — このスキルが維持するファイル。ロール一覧もここにある。なければ Step 6 で作る）
...
2. **関係者は誰か？** — この出来事を扱う人・部門・外部システムをすべて挙げてもらう。`docs/concept/CONTEXT.md` のロール節があれば既知ロールを提示し、「今回関わるのはどれか」「新しく登場するロールはいるか」の差分だけ聞く。新ロールが出たら一文定義と関心事を確認する
...
- 「概念テストの記録」の関係ロール列には `docs/concept/CONTEXT.md` のロール名をそのまま使う。**CONTEXT.md がなければ、今回登場したロールだけで新規作成する**。既にある場合も、新ロール・新しい語彙の発見があれば追記する（最初から網羅しようとしない。概念設計に登場したロールだけを都度追記する）
...
`docs/concept/CONTEXT.md` のロール節のフォーマット（表中の行は記入例）：
