...
**新規画面はない。** 卓詳細（`/game-sessions/:gameSessionId`）にセクションを1つ追加する。

### 卓詳細の「プレイメモ」セクション

| 表示要素 | 表示条件 | 内容 |
|---|---|---|
| 自分のメモ | ログインユーザー かつ その卓のメンバー | テキストエリア + 保存ボタン + 公開トグル |
| 編集不可の案内 | 上記 かつ `completed` / `cancelled` | 「卓が完了したため本文は編集できません。公開・非公開の切り替えは引き続き行えます」 |
| 公開メモ一覧 | `completed` / `cancelled` かつ 公開メモが1件以上 | メンバー名 + 本文 |
| ログイン導線 | 未ログイン または ゲスト | 「メモ機能はログインユーザー限定です」+ ログインリンク（要求 §4） |

- **公開状態がひと目で分かる**表示にする（要求 §4）。非公開が既定であることを明示し、
  公開に切り替える操作には確認を挟む（誤公開の事故を防ぐ）
- 完了・中止で本文が編集できなくなることを、**完了前から**分かるように示す（要求 §4）

### ディレクトリ構成

CLAUDE.md の「フィーチャー内のディレクトリ構成」に従い、実装詳細をサブディレクトリに隔離する。
